...
namespace MovieStoreMvc.Controllers
{
    public class ManufacturersController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name")] Manufacturer manufacturer)
        {
            ...
            {
                Manufacturer ct = await _context.Manufacturer.FirstOrDefaultAsync(c => c.Name.Equals(manufacturer.Name));
                if (ct != null)
                {
                    return Content("This Manufacturer has already been taken.");
                }
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,Name")] Manufacturer manufacturer)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    Manufacturer ct = await _context.Manufacturer.FirstOrDefaultAsync(c => c.Id != manufacturer.Id && c.Name.Equals(manufacturer.Name));
                    if (ct != null)
                    {
                        return Content("This Manufacturer has already been taken.");
                    }
